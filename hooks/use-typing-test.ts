...
const PB_KEY = "typeverse:solo-best-wpm";
const PB_RUN_KEY = "typeverse:solo-best-run";
...
export function useTypingTest() {
  ...
  useEffect(() => {
    try {
      ...
      bestRunRef.current = stored
        ? (JSON.parse(stored) as BestRunPayload)
        : null;
    } catch {
  ...
  const updateRhythm = useCallback((interval: number) => {
    ...
    const score = Math.max(
      0,
      Math.min(100, Math.round(100 - Math.max(0, stdDev - 16) * 1.8)),
    );
    setLiveRhythm(score);
  }, []);

  const trackKeyTiming = useCallback(
    (wordIdx: number, nextWordIdx: number, nextCharIdx: number) => {
      const now = Date.now();
      ...
      if (lastKeystrokeRef.current) {
        ...
        if (interval > 0 && interval < 3000) {
          ...
          const bucket = wordLatencyRef.current[wordIdx] || {
            total: 0,
            count: 0,
          };
  ...
  const endTest = useCallback(() => {
    ...
    setWordInputs((prev) => {
      setWords((currentWords) => {
        ...
        const variance =
          wpmValues.length > 0
            ? wpmValues.reduce((acc, val) => acc + (val - avg) ** 2, 0) /
              wpmValues.length
            : 0;
        const stdDev = Math.sqrt(variance);
        const consistencyScore = Math.max(
          0,
          Math.min(100, Math.round(100 - stdDev * 1.7)),
        );
      ...
      if (elapsed > 0) {
        setWordInputs((prev) => {
          setWords((currentWords) => {
            ...
            const lastRecorded =
              wpmHistoryRef.current.length > 0
      ...
      if (testState === "idle" || testState === "running") {
        ...
        if (e.key.length === 1) {
          ...
          if (!expected || e.key !== expected) {
            ...
            if (key.length === 1) {
              errorKeyMapRef.current[key] =
                (errorKeyMapRef.current[key] || 0) + 1;
            }
          }
