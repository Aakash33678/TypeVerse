  ...
  return (
    <div
      className={`pointer-events-none fixed inset-x-0 top-0 z-40 h-1 ${
        intense
          ? "bg-linear-to-r from-amber-300 via-yellow-300 to-amber-300"
          : "bg-amber-300"
      }`}
      aria-hidden="true"
    />
...
export default function RoomPage() {
  ...
  useEffect(() => {
    if (room?.status !== "finished" || !currentUserId || results.length === 0)
      return;
    const me = results.find((result) => result.playerId === currentUserId);
    if (!me) return;

    const key = "typeverse:multiplayer-best-wpm";
    ...
    return (
      <div className="flex min-h-[calc(100vh-3.5rem)] flex-col items-center justify-center bg-background">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          {isPending ? "Loading session..." : "Connecting to server..."}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-[calc(100vh-3.5rem)] flex-col items-center justify-center bg-background px-6">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            {roomError ||
              "This room may have been dissolved or does not exist."}
          </p>
    ...
    return (
      ...
      >
        <input
          ...
          className="fixed w-px h-px opacity-0 -z-10"
          tabIndex={-1}
        />
        ...
        <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-6 py-8">
          ...
          <TypingDisplay
            ...
            testState={
              typingTest.testState === "playing" ? "running" : "finished"
            }
          />
  ...
  if (room.status === "finished" && results.length > 0) {
    const isWinner =
      results.find((r) => r.placement === 1)?.playerId === currentUserId;
    const myResult = results.find((r) => r.playerId === currentUserId);
    return (
      <div className="flex min-h-[calc(100vh-3.5rem)] flex-col bg-background text-foreground">
        <WinnerCelebration
          isWinner={isWinner}
          intense={isWinner || isNewRecord}
        />
