...
export function TypingDisplay({
  ...
  return (
    <div className="relative mx-auto w-full max-w-312 rounded-2xl border border-border bg-card p-6 md:p-8">
      <div
        className="relative h-[168px] overflow-hidden font-mono text-xl leading-loose select-none md:text-2xl"
        role="textbox"
                ...
                return (
                  ...
                  >
                    {charStates.map((cs, charIdx) => {
                      ...
                      return (
                        <span key={charIdx} className="relative">
                          {isCaretHere && (
                            <span
                              className="absolute -left-px top-0.75 h-[1.25em] w-[2.5px] rounded-full bg-typing-caret animate-pulse"
                              aria-hidden="true"
                            />
                          )}
                          {isGhostCaret && (
                            <span
                              className="absolute -left-px top-0.75 h-[1.25em] w-0.5 rounded-full bg-slate-400/70"
                              aria-hidden="true"
                            />
                          )}
                          <span
                    ...
                    {isActive &&
                      ...
                      testState !== "finished" && (
                        <span
                          className="absolute -right-px top-0.75 h-[1.25em] w-[2.5px] rounded-full bg-typing-caret animate-pulse"
                          aria-hidden="true"
                        />
                      )}
                    {isGhostWord &&
                      ...
                      testState === "running" && (
                        <span
                          className="absolute -right-px top-0.75 h-[1.25em] w-0.5 rounded-full bg-slate-400/70"
                          aria-hidden="true"
                        />
                      )}
                  </div>
